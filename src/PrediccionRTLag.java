...
public class PrediccionRTLag {
    ...
    public static void main(String[] args) {
        try {
            ...
            for (int lag = 23; lag < max_lag; lag++) {
                PrintWriter writer = new PrintWriter("C:/Users/carloscharx/Documentos/Teleco/4º Teleco/Prácticas y TFG/datos-Funkfeuer-CONFINE/resultadosRTlag" + (lag+1) + ".txt");
                for (int j = 0; j < links_num; j++) {
                    ...
                    errores_acumulados[lag][j] = mae;

                    double sum = 0.0;
                    for (int k = 0; k <= j; k++) {
                        sum += errores_acumulados[lag][k];
                    }
                    sum = sum / (j + 1);
                    System.out.println(sum);
                    System.out.println("Fin de la iteración número " + (j + 1));
                    writer.println(errores_acumulados[lag][j]);
                }
                writer.close();
            }
